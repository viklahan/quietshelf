...
function SummaryBar({ result, foundCount, onCopy, copied, onNewScript, confirmingNew, onConfirmNew, onCancelNew }) {
  ...
  return (
    <div className="qfc-summary" data-comment-anchor="summary-bar">
      <div className="qfc-summary__inner">
        <div className="qfc-summary__facts">
          <span className="qfc-summary__title">{result.video_title_suggestion}</span>
          <span className="qfc-summary__meta">
            {total} segments · ≈{Eng.fmtTime(result.estimated_runtime_seconds)} runtime
          </span>
        </div>
        <div className="qfc-summary__progress">
          <ProgressBar value={total ? foundCount / total : 0} label={`${foundCount} of ${total} segments clipped`} />
        </div>
        <div className="qfc-summary__actions">
          {confirmingNew ? (
            <span className="qfc-summary__confirm">
              <span className="qfc-summary__confirmtext">Clear this map?</span>
            ...
            <span className="qfc-summary__btns">
...
function ShotList({ result, found, onFoundChange, onMapRest, dealt }) {
  return (
    <div className="qfc-board" data-screen-label="Shot List">
      <div className="qfc-board__cards">
      ...
      {result.partial ? (
        <div className="qfc-board__partial">
