...
- `engine.js` — fake mapping engine (`window.QFCEngine`). Mirrors the real backend's `POST /api/map` response shape, including `video_title_suggestion`, `estimated_runtime_seconds`, and `segments[]`. Also exports `SAMPLE_SCRIPT`.
