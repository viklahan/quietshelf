...
function Working() {
  ...
  React.useEffect(() => {
    // Cycle through messages so longer API calls (10-30s) stay animated.
    const lineTimer = setInterval(() => {
      setLineIdx((i) => (i + 1) % WORKING_LINES.length);
    }, 1800);
    const progTimer = setInterval(() => {
      setProgress((p) => Math.min(0.92, p + (0.92 - p) * 0.12));
    }, 300);
...
function EngineError({ onRetry, onBack, message }) {
  const msg = message || "The mapping engine didn't answer. Wait a moment and try again.";
  return (
    <div className="qfc-page qfc-page--center" data-screen-label="Engine Error">
      ...
      <div className="qfc-enginerror" role="alert">
        ...
        <p className="qfc-enginerror__msg">{msg}</p>
