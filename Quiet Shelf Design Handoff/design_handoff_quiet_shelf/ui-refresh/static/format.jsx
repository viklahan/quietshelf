...
/* Looks and accents for the cover Quiet Shelf makes when none is uploaded.
   Sent to the backend as cover_style + cover_accent (see BACKEND_HANDOFF.md);
   the payoff book renders the same design so what you see is what you get. */
const QS_COVER_STYLES = [
  { id: 'quiet', name: 'Quiet' },
  { id: 'frame', name: 'Frame' },
  { id: 'wash', name: 'Wash' },
  { id: 'band', name: 'Band' },
];
const QS_COVER_ACCENTS = [
  { id: 'ink', name: 'Ink', hex: null },
  { id: 'oxblood', name: 'Oxblood', hex: '#7e2b23' },
  { id: 'harbor', name: 'Harbor', hex: '#2e4257' },
  { id: 'fir', name: 'Fir', hex: '#2f5040' },
];
function qsAccentHex(accentId, theme) {
  const a = QS_COVER_ACCENTS.find((x) => x.id === accentId);
  if (!a || !a.hex) return (QS_COVER_PALETTE[theme] || QS_COVER_PALETTE.classic).ink;
  return a.hex;
}
...
function Format() {
  ...
  const [coverStyle, setCoverStyle] = React.useState('quiet');
    ...
    try {
      const [out] = await Promise.all([
        window.QS_API.formatBook({
          ...
          coverAccent: qsAccentHex(accent, theme),
        }),
  ...
  function reset() {
    ...
    setTitle(''); setAuthor(''); setTheme('classic'); setCoverStyle('quiet'); setAccent('ink'); setError(''); setResult(null);
  }
        ...
        <div className="qs-shelfwrap qs-shelfwrap--lg">
          <Shelf lit={true}>
            <FinishedBook
              ...
              face={theme}
              coverStyle={coverStyle}
              accent={qsAccentHex(accent, theme)}
            />
          </Shelf>
        </div>
  ...
  return (
    <div className="qs-page qs-page--narrow">
      ...
      <div className="qs-step">
        ...
        {storyName ? (
          ...
          <div className="qs-markwrap">
            <span className="qs-markwrap__ico" aria-hidden="true"><QSIcon name="book-open" size={120} /></span>
      ...
      <div className="qs-step">
        ...
        {!coverName ? (
          <div className="qs-covpick">
            <div className="qs-covpick__row" role="radiogroup" aria-label="Cover look">
              <span className="qs-covpick__label">The look</span>
              {QS_COVER_STYLES.map((s) => {
                const pal = QS_COVER_PALETTE[theme] || QS_COVER_PALETTE.classic;
                return (
                  <button key={s.id} type="button" role="radio" aria-checked={coverStyle === s.id}
                    className={`qs-covmini qs-covmini--${s.id}${coverStyle === s.id ? ' qs-covmini--on' : ''}`}
                    style={{ '--cov-bg': pal.bg, '--cov-ink': pal.ink, '--cov-accent': qsAccentHex(accent, theme) }}
                    onClick={() => setCoverStyle(s.id)} aria-label={`Cover look: ${s.name}`} title={s.name}>
                    <span className="qs-covmini__rule" aria-hidden="true"></span>
                    <span className="qs-covmini__t" aria-hidden="true"></span>
                    <span className="qs-covmini__t2" aria-hidden="true"></span>
                  </button>
                );
              })}
            </div>
            <div className="qs-covpick__row" role="radiogroup" aria-label="Cover accent color">
              <span className="qs-covpick__label">The accent</span>
              {QS_COVER_ACCENTS.map((a) => (
                <button key={a.id} type="button" role="radio" aria-checked={accent === a.id}
                  className={`qs-accent${accent === a.id ? ' qs-accent--on' : ''}`}
                  style={{ background: a.hex || (QS_COVER_PALETTE[theme] || QS_COVER_PALETTE.classic).ink }}
                  onClick={() => setAccent(a.id)} aria-label={`Accent: ${a.name}`} title={a.name}></button>
              ))}
            </div>
          </div>
        ) : null}
